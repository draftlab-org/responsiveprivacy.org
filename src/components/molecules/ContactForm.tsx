...
const inputOk = 'border-gray-300 focus:border-blue-500 focus:ring-blue-500';
...
export default function ContactForm() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!altcha) {
      setError(
        'Please wait a moment for verification to finish, then try again.'
      );
      setStatus('error');
      return;
    }
    ...
    >
        ...
        <div className="flex-1 text-sm text-gray-700">
          <label className="flex items-start gap-2">
            <input
              ...
              disabled={submitting}
              className="text-blue-600 focus:ring-blue-500 mt-1 size-4 shrink-0 rounded border-gray-300"
            />
            <span>
              I'd also like to receive occasional updates about Responsive
              Privacy.
            </span>
          </label>
          <p className="mt-2 text-xs text-gray-600">
            We'll use your email exclusively for Responsive Privacy updates. You
            can unsubscribe at any time. Your details are stored with{' '}
            <a
      ...
      <altcha-widget
        ...
        style={{
          ...
          '--altcha-color-base': 'rgb(249 250 251)',
          '--altcha-border-color': 'rgb(229 231 235)',
